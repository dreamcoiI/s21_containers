...
class RBTree {
 ...
 public:
  ...
  // А для нахождения максимального кол-ва эл-ов мы делим полученное значение на
  // sizeof одного узла
  ...
  //Итератор для вставки в контейнер элемента, если контейнер не содержит такого
  //элемента. Используем pair для пару ключ и булева переменная(true-получилось
  //вставить, false-нет)
  ...
  struct RedBlackNode {
    ...
    tree_node *NodeNext() const noexcept {
      ...
      if (node->color_ == tRed &&
        ...
        //Если левых ветвей нет-то самым маленьким значением является значение в
        //котором мы сейчас
        while (node->left_ != nullptr) node = node->left_;
      } else {
        //в остальных случаях идем по родительским узлам, до тех пор пока правая
        //ветвь родителя не будет содержать узел отличный от текущего
